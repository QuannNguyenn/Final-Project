...
def clean_state_and_flag(col):
    ...
    # Normalize casing
    upper_col = col.str.strip().str.upper()
    
    # Map state codes
    mapped = upper_col.map(state_map)
    
    # If not mapped, keep original cleaned value
    ...
    # US detection
    us_states = {v.title() for v in state_map.values()}
...
transactions_data_df["zip"] = (
    ...
    .fillna(0)             # Temporary fill to allow integer conversion
    .astype(int)           # Remove decimals
    .astype(str)           # Back to string
    .replace("0", "Not Available") # Turn our temp 0 into the label
)
...
cursor.execute("""
    IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('transformation.transactions_data') AND type = 'U')
        BEGIN
...
cursor.execute("TRUNCATE TABLE transformation.transactions_data")

# Save your cleaned dataframe to a local file
# Use 'index=False' and 'header=False' for SQL compatibility
temp_csv_path = r"G:\temp_data_engineering\temp_cleaned_data.csv"

print("Exporting cleaned data to temporary CSV...")
transactions_data_df.to_csv(temp_csv_path, index=False, header=False, sep='|', encoding='utf-8',lineterminator='\n') 
print("Export complete.")

bulk_insert_query = f"""
BULK INSERT transformation.transactions_data
FROM '{temp_csv_path}'
